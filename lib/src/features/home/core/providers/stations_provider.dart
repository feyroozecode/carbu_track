...
final filteredStationsProvider = Provider<List<Station>>((ref) {
  ...
  return stations
      .where((station) => station.fuelTypes.contains(selectedFuelType))
      .toList();
});
...
final nearestStationProvider = Provider<Station?>((ref) {
  ...
  double shortestDistance = distance.as(LengthUnit.Kilometer, userLocation,
...
final distanceToNearestStationProvider = Provider<double?>((ref) {
  ...
  final distance = Distance();
  return distance.as(LengthUnit.Kilometer, userLocation,
      LatLng(nearestStation.latitude, nearestStation.longitude));
});
  ...
  Future<void> loadStations() async {
    try {
      ...
      final String response =
          await rootBundle.loadString('lib/common_data/main/stations.geojson');
